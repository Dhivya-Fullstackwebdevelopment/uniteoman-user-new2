  ...
  return (
    <main className="min-h-screen pt-[0px] bg-white overflow-hidden">
      ...
      <div className="shelves pb-[48px] ">
        <div className="c ">
          {trending.length > 0 && <ServiceShelf title="Trending this week" items={trending} cardType="A" seeAllLink="/search?sort=featured"/>}
          {justAdded.length > 0 && <ServiceShelf title="Just added" items={justAdded} cardType="A"  seeAllLink="/search?sort=newest"/>}
          {beauty.length > 0 && <ServiceShelf title="Beauty & wellness for women" items={beauty} cardType="B"   seeAllLink="/search?category=spa"/>}
          {tech.length > 0 && <ServiceShelf title="Tech & Software" items={tech} cardType="B" seeAllLink="/search?category=it-software"/>}
          {homeEssentials.length > 0 && <ServiceShelf title="Home Essentials" items={homeEssentials} cardType="C"  seeAllLink="/search?category=retail"/>}
          {grooming.length > 0 && <ServiceShelf title="Grooming for men" items={grooming} cardType="B"  seeAllLink="/search?category=grooming-for-men"/>}
        </div>
      </div> */}
      <TrustBanner />
      <CategoryGrid />
