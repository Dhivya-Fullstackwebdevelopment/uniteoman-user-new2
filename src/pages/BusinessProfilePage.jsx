    ...
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map(n => (
        <button
          ...
          onClick={() => onChange(n)}
          className={`text-2xl transition-all ${n <= (hover || value) ? 'text-[#e8317a] scale-110' : 'text-gray-200'
            }`}
        >
          ★
        </button>
  ...
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((n) => (
        <Star
          key={n}
          className={`w-3.5 h-3.5 ${n <= Math.round(rating) ? 'fill-[#e8317a] text-[#e8317a]' : 'text-gray-200'
            }`}
        />
      ))}
    </div>
...
export default function BusinessProfilePage() {
  ...
  const { data: services = [] } = useQuery({
    ...
    queryFn: () =>
      serviceApi.listByBusiness(business.id),
    enabled: !!business?.id
  })
  ...
  const { name_en, description, short_description, category, governorate,
    ...
    business_hours: rawHours = {}, tags: rawTags = [], services: rawServices = [], owner_id, booking_count } = business
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    onClick={() => {
                      if (!isCustomerLoggedIn) {
                        ...
                        return navigate(
                          `/customer/login?redirect=${encodeURIComponent(location.pathname)}`
                        )
                      }
                      handleWhatsApp()
                    }}
                ...
                {website && (
                  <button
                    ...
                    className="flex-1 min-w-[100px] flex items-center justify-center gap-2 px-4 py-2.5 bg-gray-100 text-gray-700 rounded-xl text-sm font-medium hover:bg-gray-200 transition-colors"
                  >
                    <Globe size={16} />
                    Website
                  </button>
                  ...
                  <div>
                    ...
                    {has_deal && (
                      <div className="mt-4 p-4 bg-[#FFF3E0] rounded-xl">
                        <span className="text-sm font-semibold text-[#e8317a]">
                          🎁 Special Deal
                        </span>

                        <p className="text-sm text-gray-700 mt-1">
                          {deal_text || 'Limited time offer available!'}
                        </p>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 sticky top-20">
              ...
              <button
                onClick={() => {
                  // Define the exact target route where the customer needs to go
                  const bookingPath = `/business/${slug}/book`;

                  if (!isCustomerLoggedIn) {
                    toast.error('Please login to continue');

                    // FIX: Use bookingPath here so the login page knows where to redirect after success
                    return navigate(
                      `/customer/login?redirect=${encodeURIComponent(bookingPath)}`
                    );
                  }

                  // If already logged in, navigate straight to the booking page
              ...
              <div className="mt-6 pt-6 border-t border-gray-100">
                <div className="grid grid-cols-2 gap-3 text-center">
                  ...
                  <div>
                    <div className="text-lg font-bold text-gray-900"> {booking_count || 0}</div>
