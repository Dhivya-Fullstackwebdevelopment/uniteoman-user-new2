...
const editShopSchema = z.object({
  name_en: z.string().min(1, 'Shop name is required'),
})
...
export default function EditShopPage() {
  ...
  const handleSubmit = (e) => {
    ...
    const result = editShopSchema.safeParse(form)
      ...
      <form id="edit-form" onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-8 space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block">Shop Name (English)<span className='text-red-500'>*</span></label>
              <input value={form.name_en}
                ...
                className="rounded-xl px-4 py-3 text-sm outline-none focus:border-pink transition-colors" />
              {errors.name_en && (
            ...
            <div>
              ...
              <input value={form.name_ar} onChange={e => setForm({ ...form, name_ar: e.target.value })}
                style={{ border: '1px solid #dfdee5' }}
                className="w-full rounded-xl px-4 py-3 text-sm outline-none focus:border-pink transition-colors text-right" dir="rtl" />
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              ...
              <select value={form.category_id} onChange={e => setForm({ ...form, category_id: e.target.value })}
                style={{ border: '1px solid #dfdee5' }}
                className="w-full rounded-xl px-4 py-3 text-sm outline-none focus:border-pink transition-colors bg-white font-semibold">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })}
                  placeholder="+968 ..."
                  style={{ border: '1px solid #dfdee5' }}
                  className="w-full  rounded-xl pl-10 pr-4 py-3 text-sm outline-none focus:border-pink transition-colors" />
              </div>
            </div>
            <div>
            ...
            {Object.entries(form.business_hours).map(([day, hours]) => (
              <div key={day} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-2xl bg-gray-50/50 border border-gray-100 gap-4">
                ...
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  })} className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all ${hours.closed ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                    }`}>
